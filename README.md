...
You can configure PhantomJS for browser tests using `markoDevTools.config.phantomOptions`.
...
You can easily configure Mocha for server-side tests using `markoDevTools.config.mochaOptions`.
...
```javascript
module.exports = function(markoDevTools) {
    markoDevTools.config.mochaOptions = {
        timeout: 5000,
        colors: true
    };
}
